...
input_list = ["UPDATE_FREQ", "NEEDLES"]
# list of default text
default_list = [10, 1000]
output = multenterbox("Buffon's Needle Problem\nCreated By: M.Torkaman Dehnavi & S.H. Mirhosseini 11/2021\nhosseinmirhosseini76@gmail.com\nmtorkaman69@gmail.com\n"
                      , "Buffon's Needle Problem", input_list, default_list)
UPDATE_FREQ = int(output[0])
NEEDLES = int(output[1])
...
class Buffon_Sim:
    ...
    def __init__(self):
        ...
        fig.canvas.manager.set_window_title(window)  # window title
    ...
    def plotFloorboards(self):
        ...
        for j in range(self.boards):  # for every floorboard
            ...
            self.buffon.hlines(y=self.floorboards[j], xmin=0, xmax=BOUND, color=FLOORBOARD_COLOR, linestyle='--',
                               linewidth=2.0)  # horizontal lines, with x limits
    ...
    def updateResults(self, needlesTossed=0):
        if self.intersections == 0:  # to avoid division by 0 error
            ...
            sim_pi = (2 * NEEDLE_LENGTH * needlesTossed) / (
                        FLOORBOARD_WIDTH * self.intersections)  # from wikipedia page
    ...
    def plot(self):
        ...
        self.buffon.legend(legend_lines, ['floorboard', 'intersecting needle', 'non-intersecting needle'], loc=1,
